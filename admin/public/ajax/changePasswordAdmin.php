...
if($_SERVER['REQUEST_METHOD'] === "POST"){
    ...
    if(!empty($_POST['mail']) and is_string($_POST['mail']) and !empty($_POST['password']) and is_string($_POST['password']) and strlen($_POST['password']) >=6){
        ...
        if(isset($_POST['currentPassword'])){
            ...
            $query1 ="SELECT password FROM admin WHERE mail = '$userMail'";
        ...
        $query1 = "UPDATE admin SET klucz = '".randomString()."', password_temporary = '$password_temporary' WHERE mail = '$userMail'";
        $stmt1 = $conn->prepare($query1);
        $error1 = $stmt1->execute();

        //pobranie klucza z bazy
        $query2 = "SELECT klucz, mail, id FROM admin WHERE mail ='$userMail'";
        ...
        if(!empty($result1)){
            ...
            $link = $_SERVER['SERVER_NAME'].'/admin/reset/'.$id.'/'.$klucz;
