...
require_once dirname(dirname(dirname(dirname(__FILE__)))).'/app/assets/PHPMailer-master/src/Exception.php';
require_once dirname(dirname(dirname(dirname(__FILE__)))).'/app/assets/PHPMailer-master/src/PHPMailer.php';
require_once dirname(dirname(dirname(dirname(__FILE__)))).'/app/assets/PHPMailer-master/src/SMTP.php';
...
//id- id komunikatu z bazy danyc tabela messages
//recepient - adres email
//value - wartosc zastepowana z bazy danych
//atachment  = zalaczmik
    public function valueMail($id, $recipient, $value = false, $atachment = false){
        ...
        $subjectDatabase = $val['topic'];
        $textMail = $val['text'];
        $html = $val['isHTML'];

        if($value !== false){
            $text = str_replace("%value%", $value, $textMail);
            $subject = str_replace("%value%", $value, $subjectDatabase);
        }
        else {
            $text = $textMail;
            $subject = $subjectDatabase;
        }
	...
	<div class="container">
		...
		<div class="content">
			<div class="dataContent floatLeft">
				<p class="darkGrayColor">'.$companyName.'<p>
