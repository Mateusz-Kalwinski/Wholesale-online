...
if($_SERVER['REQUEST_METHOD'] === "POST") {
    if (!empty($_POST['delivery']) and !empty($_POST['place']) and !empty($_POST['postal']) and !empty($_POST['name'])
        ...
        if (!empty($result3)) {
            ...
            foreach ($result3 as $cartData) {
                if ($case < $cartData['ilosc']) {
                    ...
                    $changedProductsList.=$cartData['name'] . ' z ' . $cartData['ilosc'] .' '.$cartData['unit'].
                            ' zmieniono na ' . $subtraction .' '.$cartData['unit'];
            ...
            do {
                ...
                if (empty($result1)) {
                    ...
                    $userDiscount=$result3[0]['user_discount'];
                    $query2 = "INSERT INTO orders_history (name, order_number, order_date, pay, shipment, order_status, order_paid, product, place, code, address, phone, comment, id_user,`user_discount`) VALUES
                              ('$name', '$orderNumber', '$date', '$productPrice', '$shipment', '2', '1', '$product', '$place', '$code', '$address', '$phone', '$comment', '$id',$userDiscount)";
                    $stmt2 = $conn->prepare($query2);
                    $error2 = $stmt2->execute();
        ...
        if (!empty($result4)) {
            ...
            if (!empty($notifications[24]['text'])) {
                ...
                $notification= str_replace(['%numer%','%mail%'],[$orderNumber,$recipient],$notifications[24]['text']);
                
                
                $responseMessage = htmlentities('[SUKCES]' . $notification . '<br>' . $responseMessage);

            }else{
                echo 'Twój koszyk jest pusty.<br> <a href="http://test.pl/promocje"> Wróc do przeglądania produktów</a> ';
            }
        }
    }
    else
    {
        if(!empty($notifications[25]['text'])) {
            $responseMessage = $notifications[25]['text'];
        }else{
            echo "Wszystkie pola powinny być wypełnione";
        }
    }
    echo $responseMessage;
}