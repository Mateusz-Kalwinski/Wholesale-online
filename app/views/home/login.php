...
<body class="site-login">
    <main>
        <div class="login-panel">
            <form class="showed login-form" action="public/ajax/login.php" method="post">
                ...
                <p>Nie pamiętasz hasła? <b class="login-recover-password">Przywróć hasło.</b></p>
                        ...
                        <form class="recover-password " action="public/ajax/changePassword.php" method="post">
                <img src="/public/images/personal/logo.png" alt="Logo firmy">
    ...
    <script>
        $('.login-panel form.login-form').submit(function(e)
        {
            ...
            $.post('/public/ajax/login.php',{'mail':username,'password':password}).done(function(data)
            {
                ...
                {
                    window.location.replace("/"+data.substr(1));
                }
                else if(response==2)
                {
        ...
        $('.login-panel form.recover-password').submit(function(e)
        {
            ...
            var password=$(this).find('input[name="password"]').val();
            $.post('/public/ajax/changePassword.php',{'mail':mail,'password':password}).done(function(data)
            {
